...
import { AlertCircle, BadgeCheck, BookOpen, ExternalLink, Languages, Search } from "lucide-react";
  ...
  return (
    <AppShell>
      <div className="space-y-4">
        <header className="rounded-xl bg-primary-container p-5 text-primary-container-foreground">
          <p className="label-sm flex items-center gap-1.5 opacity-90">
            <BookOpen className="size-4" aria-hidden />A civic reminder
          </p>
          <h1 className="mt-1 text-2xl font-semibold leading-tight tracking-tight">
            The Constitution of India
          </h1>
          <p className="mt-2 text-sm leading-relaxed opacity-90">
            Every road, pipeline and hospital in this app exists because of powers and duties
            written down here. Read it slowly. Nothing on this page is our wording — the text comes
            from the official published Constitution, and we say plainly when we cannot confirm a
            language version.
          </p>
        </header>
          ...
          {pickerOpen ? (
            <div className="mt-4 border-t border-border pt-3">
              ...
              <ul className="mt-3 max-h-80 space-y-1.5 overflow-y-auto pr-1">
                  ...
                  return (
                    <li key={l.code}>
                      <button
                        ...
                        className={cn(
                          "m3-state flex w-full items-center justify-between gap-3 rounded-lg border px-3 py-2.5 text-left",
                          active
                      ...
                      >
                        <span className="min-w-0">
                          <span className="block truncate text-sm font-medium">{l.nativeName}</span>
                ...
                {languages.length === 0 ? (
                  <li className="py-3 text-sm text-muted-foreground">No language matches that.</li>
                ) : null}
              </ul>
              <p className="mt-3 text-xs text-muted-foreground">
                The 22 languages listed in the Eighth Schedule are all shown here. We only mark a
                version as ready when we have checked it against an official source, so most are
          ...
          {language.preamble ? (
            <article>
              ...
              <p className="mt-5 border-t border-border pt-3 text-xs text-muted-foreground">
                Source: {language.source.label}. Adopted 26 November 1949; came into force 26
                January 1950.
              </p>
            </article>
          ) : (
            <div className="mx-auto max-w-prose text-center">
              ...
              <p className="mt-2 text-sm leading-relaxed text-muted-foreground">{language.note}</p>
              <div className="mt-4 flex flex-wrap justify-center gap-2">
                <Button asChild className="rounded-full">
                  <a href={OFFICIAL_SOURCES.regional.url} target="_blank" rel="noreferrer noopener">
                    Open the official language list
                  </a>
                </Button>
                <Button variant="outline" className="rounded-full" onClick={() => choose("en")}>
                  Read in English
                </Button>
              </div>
            </div>
        ...
        <section className="space-y-3">
          <div>
            <h2 className="display-sm">Find your way around</h2>
            <p className="text-sm text-muted-foreground">
              The Constitution is organised into Parts. Search a topic, a Part or an article number.
            </p>
          </div>
          ...
          {articles.length > 0 ? (
            <div>
              ...
              <ul className="mt-2 space-y-2">
                {articles.map((a) => (
                  <li key={a.article} className="rounded-lg bg-surface-container p-3.5">
          ...
          {parts.length > 0 ? (
            <div>
              ...
              <ol className="mt-2 space-y-2">
                {parts.map((p) => (
                  <li key={p.part} className="rounded-lg border border-border p-3.5">
          ...
          {parts.length === 0 && articles.length === 0 ? (
            <p className="rounded-lg bg-surface-container p-4 text-sm text-muted-foreground">
              Nothing here matches “{query}”. The full official text has far more detail — open the
              official Constitution page to search all of it.
            </p>
          ) : null}
        </section>
        ...
        <section className="rounded-xl border border-border bg-surface p-4">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            We do not host our own copy of the Constitution. Everything above points back to the
            Legislative Department, Ministry of Law and Justice, which publishes the official text.
          </p>
